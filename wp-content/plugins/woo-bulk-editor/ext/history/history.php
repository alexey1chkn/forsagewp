...
final class WOOBE_HISTORY extends WOOBE_EXT {
    ...
    public function __construct() {
        ...
        //tabs
        $this->add_tab($this->slug, 'panel', __('History', 'woo-bulk-editor'));
    ...
    public function woobe_ext_scripts() {
        ...
        <script>
            ...
            lang.<?php echo $this->slug ?>.reverting = '<?php _e('Reverting', 'woo-bulk-editor') ?> ...';
            lang.<?php echo $this->slug ?>.reverted = '<?php _e('Reverted!', 'woo-bulk-editor') ?>';
            lang.<?php echo $this->slug ?>.wait_until_finish = '<?php _e('Wait please while data reverting is going!', 'woo-bulk-editor') ?>';
            lang.<?php echo $this->slug ?>.clearing = '<?php _e('History clearing ...', 'woo-bulk-editor') ?>';
            lang.<?php echo $this->slug ?>.cleared = '<?php _e('History is cleared!', 'woo-bulk-editor') ?>';
            lang.<?php echo $this->slug ?>.history_is_going = "<?php echo __('ATTENTION: History operation is going!', 'woo-bulk-editor') ?>";
        </script>
        <?php
    }
  ...
  KEY `user_id` (`user_id`)
        ...
        if ($wpdb->query($sql1) === false) {
            ...
            <div class="error notice">
                <p class="description"><?php _e("WOOBE cannot create the database table! Make sure that your mysql user has the CREATE privilege! Do it manually using your host panel phpmyadmin!", 'woo-bulk-editor') ?></p>
